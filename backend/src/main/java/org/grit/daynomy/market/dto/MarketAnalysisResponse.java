...
public record MarketAnalysisResponse(
  ...
  public static MarketAnalysisResponse from(MarketAnalysis marketAnalysis) {
    return new MarketAnalysisResponse(
        marketAnalysis.getCause(),
        marketAnalysis.getAssets().getValues().stream().map(AssetImpactResponse::from).toList(),
        marketAnalysis.getScenarios().getValues().stream().map(ScenarioResponse::from).toList());
  }
}
