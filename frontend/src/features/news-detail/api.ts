import { mockImpacts, mockNews, mockRelatedIssues } from './mock'
import type { Impact, NewsDetail, NewsDetailPayload, RelatedIssue } from './types'
...
function normalizeNews(raw: Partial<NewsDetail> & Record<string, unknown>): NewsDetail {
  return {
    ...
    originalUrl:
      typeof raw.originalUrl === 'string' ? raw.originalUrl : mockNews.originalUrl,
    summary: String(raw.summary ?? mockNews.summary),
    body: Array.isArray(raw.body)
      ? raw.body.map(String)
      : String(raw.content ?? raw.originalText ?? '').split('\n').filter(Boolean),
    imageUrl: typeof raw.imageUrl === 'string' ? raw.imageUrl : undefined,
  }
}

function normalizeRelatedIssues(raw: Array<Partial<RelatedIssue> & Record<string, unknown>>): RelatedIssue[] {
  return raw.map((issue) => ({
    keyword:
      typeof issue.keyword === 'string' ? issue.keyword : String(issue.title ?? ''),
    title: String(issue.title ?? issue.keyword ?? '키워드'),
    probability:
      typeof issue.probability === 'number' ? issue.probability : undefined,
...
export async function getNewsDetail(newsId: string): Promise<NewsDetailPayload> {
  const [news, impacts, relatedIssues] = await Promise.allSettled([
    ...
    getJson<Array<Partial<RelatedIssue> & Record<string, unknown>>>(
